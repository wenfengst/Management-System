...
def create_render(privilege): 
    if logged():  
        if privilege==1:  
            render = web.template.render('templates/admin', base='base', globals={'datestr': web.datestr,})
        elif privilege==2:  
            render = web.template.render('templates/manager', base='base', globals={'datestr': web.datestr,})
        elif privilege==3:  
            render = web.template.render('templates/organizer', base='base', globals={'datestr': web.datestr,})
    else:  
        render = web.template.render('templates', base='base', globals={'datestr': web.datestr,})
    return render  
...
class Index_class:
    ...
    def GET(self,term):
        if session.privilege==3:
            wh = ' u2.uid='+str(session.uid)
        elif session.privilege==2:
            wh = ''
        if term=='':
            classes = model.get_classes('where'+wh+' order by ctime DESC')
        elif term=='unapprove':
            classes = model.get_classes('where approve=\'未审批\' and'+wh+' order by ctime DESC')
        elif term=='pass':
            classes = model.get_classes('where approve=\'通过\' and'+wh+' order by ctime DESC')
        elif term=='notpass':
            classes = model.get_classes('where approve=\'不通过\' and'+wh+' order by ctime DESC')
        render = create_render(session.privilege)
