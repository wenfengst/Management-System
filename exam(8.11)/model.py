...
def update_project_apply(pid,oid):
    db.update('project',where='pid=$pid',vars=locals(),oid=oid,apply='已申请',ptime=datetime.datetime.now())

def update_project_approve(cid,mid):
    db.update('class',where='cid=$cid',vars=locals(),mid=mid,approve='已审批',ctime=datetime.datetime.now())
...
def get_classes():
    ...
    inner join classroom on class.crid=classroom.crid left join manager on class.mid=manager.mid order by ctime DESC')
    
def get_pclasses(pid):
    return db.query('select cid,cname,begindate,begintime,enddate,endtime,classroom.place,classroom.classroom,\
    project.pname,teacher.tname,approve,manager.mname,ctime \
    from class inner join project on class.pid=project.pid inner join teacher on class.tid=teacher.tid \
    inner join classroom on class.crid=classroom.crid left join manager on class.mid=manager.mid \
    where class.pid=$pid order by ctime DESC',vars=locals())
