...
session = web.session.Session(app, web.session.DiskStore('sessions'),initializer={'login':0,'uid':0,'privilege':0,})#设置session的存储方式为磁盘.initializer 指定session的初始化值
...
class Login:
    ...
    def GET(self):
        '''if logged():
            ...
            render1 = create_render(session.privilege)
            return render1.index(user,datetime.datetime.now().strftime('%Y-%m'))
        else: ''' 
    ...
    def POST(self):
        ...
        if len(users)==1:
            ...
            if usertype=='administrator':
                ...
                session.privilege=3
            render = create_render(session.privilege)
            return render.index(users[0],datetime.datetime.now().strftime('%Y-%m'))
        else:
